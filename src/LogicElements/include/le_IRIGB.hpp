...
public:
     ...
     */
    IRIGB(uint32_t irigTimerFreq, float irigFrameTolerance, uint32_t updateTimerFreq);
...
private:
     ...
     */
    inline IRIGBFrame DecodeFrame(uint16_t frame);
     ...
     */
    void DecodeFrames(IRIGBFrame* frames, uint16_t offset);
     ...
     */
    inline uint16_t FromBCD(IRIGBFrame* buffer, uint16_t start, uint16_t stop, uint16_t multiplier);
    ...
    IRIGBFrame* _irigFrameIn; ///< Frame buffer input
    IRIGBFrame* _irigFrameOut; ///< Frame buffer output, left-aligned
