...
serialib::serialib()
{
    ...
    currentStateRTS = true;  /**< Set default value for RTS (Windows only) */
    currentStateDTR = true;  /**< Set default value for DTR (Windows only) */
...
/*!
    \brief      Destructor of the class serialib. It closes the connection.
*/
serialib::~serialib()
{
    closeDevice();
...
char serialib::openDevice(const char *Device, const unsigned int Bauds,
    ...
    // Configure byte size, stop bits, and parity
    ...
    options.c_cc[VMIN] = 0;  // At least one character before satisfying reading
...
int serialib::writeChar(const char Byte)
{
    ...
    if (!WriteFile(hSerial, &Byte, 1, &dwBytesWritten, NULL)) return -1; /**< Return -1 if an error occurred */
    return 1; /**< Write operation successful */
#endif
#if defined (__linux__) || defined(__APPLE__)
    if (write(fd, &Byte, 1) != 1) return -1; /**< Return -1 if an error occurred */
    return 1; /**< Write operation successful */
#endif
}
...
/*!
    ...
    \param receivedString : string to send on the port
    \return 1 success
    \return -1 error while writing data
*/
int serialib::writeString(const char *receivedString)
{
    ...
    if (!WriteFile(hSerial, receivedString, strlen(receivedString), &dwBytesWritten, NULL))
        return -1; /**< Error while writing, return -1 */
    return 1; /**< Write operation successful */
#endif
#if defined (__linux__) || defined(__APPLE__)
    int Length = strlen(receivedString); /**< Length of the string */
    if (write(fd, receivedString, Length) != Length) return -1; /**< Return -1 if an error occurred */
    return 1; /**< Write operation successful */
#endif
}
...
#endif
#if defined (__linux__) || defined(__APPLE__)
    if (write(fd, Buffer, NbBytes) != (ssize_t)NbBytes) return -1; /**< Return -1 if an error occurred */
    return 1; /**< Write operation successful */
#endif
}
...
/*!
    ...
    \param timeOut_ms : delay of timeout before giving up the reading
            If set to zero, timeout is disabled (Optional)
    \return 1 success
...
int serialib::readChar(char *pByte, unsigned int timeOut_ms)
{
    ...
    COMMTIMEOUTS timeouts = {0}; /**< Initialize the COMMTIMEOUTS structure */
    ...
    if (!SetCommTimeouts(hSerial, &timeouts)) return -1; /**< Return -1 if an error occurred */

    if (!ReadFile(hSerial, pByte, 1, &dwBytesRead, NULL)) return -2; /**< Return -2 if an error occurred */

    if (dwBytesRead == 0) return 0; /**< Return 0 if the timeout is reached */

    return 1; /**< The byte is read */
#endif
#if defined (__linux__) || defined(__APPLE__)
...
{
    unsigned int NbBytes = 0; /**< Number of characters read */
    char charRead; /**< Returned value from Read */
    ...
    while (NbBytes < maxNbBytes) {
        ...
        if (charRead == 1) {
            if (receivedString[NbBytes] == finalChar) {
                receivedString[++NbBytes] = 0; /**< Add zero (end of string) */
                return NbBytes; /**< Return the number of bytes read */
            }
            NbBytes++; /**< Increase the number of bytes read */
        }

        if (charRead < 0) return charRead; /**< Return the error number */
    }
...
/*!
    ...
    \return -1 error while reading the character
    \return -3 MaxNbBytes is reached
*/
int serialib::readString(char *receivedString, char finalChar, unsigned int maxNbBytes, unsigned int timeOut_ms)
{
    if (timeOut_ms == 0) return readStringNoTimeOut(receivedString, finalChar, maxNbBytes); /**< Check if timeout is requested */

    unsigned int nbBytes = 0; /**< Number of bytes read */
    char charRead; /**< Character read on serial device */
    timeOut timer; /**< Timer used for timeout */
    long int timeOutParam;

    timer.initTimer(); /**< Initialize the timer (for timeout) */

    while (nbBytes < maxNbBytes) {
        timeOutParam = timeOut_ms - timer.elapsedTime_ms(); /**< Compute the TimeOut for the next call of ReadChar */

        if (timeOutParam > 0) {
            charRead = readChar(&receivedString[nbBytes], timeOutParam); /**< Wait for a byte on the serial link with the remaining time as timeout */

            if (charRead == 1) {
                if (receivedString[nbBytes] == finalChar) {
                    receivedString[++nbBytes] = 0; /**< Add the end character 0 */
                    return nbBytes; /**< Return the number of bytes read */
                }
                nbBytes++; /**< Increase the number of bytes read */
            }
        ...
        if (timer.elapsedTime_ms() > timeOut_ms) {
            receivedString[nbBytes] = 0; /**< Add the end character */
            return 0; /**< Return 0 (timeout reached) */
        }
    }
    return -3; /**< Buffer is full */
}
...
int serialib::readBytes(void *buffer,unsigned int maxNbBytes,unsigned int timeOut_ms, unsigned int sleepDuration_us)
{
    ...
    // Initialize the COMMTIMEOUTS structure
    COMMTIMEOUTS timeouts = {0}; /**< Initialize the COMMTIMEOUTS structure */
    timeouts.ReadIntervalTimeout = MAXDWORD;
    timeouts.ReadTotalTimeoutConstant = timeOut_ms;
    timeouts.ReadTotalTimeoutMultiplier = 0;
    ...
    // Read the bytes from the serial device, return -2 if an error occurred
    if (!ReadFile(hSerial, buffer, maxNbBytes, &dwBytesRead, NULL)) return -2;
