...
{
    strncpy(args[0].sArg, echo.c_str(), MSG_REQ_ARG_MAX_LENGTH);
    args[0].sArg[MSG_REQ_ARG_MAX_LENGTH - 1] = '\0';
}
...
{
    strncpy(args[0].sArg, elementName.c_str(), MSG_REQ_ARG_MAX_LENGTH);
    args[0].sArg[MSG_REQ_ARG_MAX_LENGTH - 1] = '\0';
    ...
    : msg(type, msg_category::RESPONSE_PARTIAL), badResponse(badResponse)
{
    ...
    if (length < MSG_RESP_MAX_LENGTH)
    {
        ...
        this->length = length;
        this->category = msg_category::RESPONSE_COMPLETE;
    }
    else
    {
