 ...
 */
std::string le_AbstractConnectionHandler::ReadLine()
{
    if(lineBuffer.empty())
    {
        ReadToBuffer();
 ...
 */
uint16_t le_AbstractConnectionHandler::WriteLine(const std::string& line)
{
 ...
 */
void le_AbstractConnectionHandler::SetCommandInProcess(bool inProcess)
{
 ...
 */
void le_AbstractConnectionHandler::HandleSpecialKey(char key)
{
    if (key == 'A') // Up arrow
    {
        if (historyIndex > 0)
 ...
 */
bool le_AbstractConnectionHandler::getEscapeKeyRequested() const
{
 ...
 */
void le_AbstractConnectionHandler::acknowledgeEscapeKeyRequest()
{
 ...
 */
le_TcpConnectionHandler::le_TcpConnectionHandler(MinimalSocket::tcp::TcpConnectionBlocking& connection)
 ...
 */
uint16_t le_TcpConnectionHandler::read(char* buffer, size_t size) 
{
 ...
 */
uint16_t le_TcpConnectionHandler::write(const char* buffer, size_t size) 
{
 ...
 */
void le_TcpConnectionHandler::close() 
{
    connection.shutDown();
}
 ...
 */
le_SerialConnectionHandler::le_SerialConnectionHandler(serialib* connection)
 ...
 * @return The number of bytes read.
 */
uint16_t le_SerialConnectionHandler::read(char* buffer, size_t size) 
{
    //ioLock.lock();
 ...
 * @return The number of bytes written.
 */
uint16_t le_SerialConnectionHandler::write(const char* buffer, size_t size) 
{
    //ioLock.lock();
 ...
 */
void le_SerialConnectionHandler::close() {}