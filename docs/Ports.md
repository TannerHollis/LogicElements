...
| `LE_NS_PER_ABSTRACT_CYCLE` | Port-calibrated worst-case **nanoseconds per compiler abstract cycle** (the `.leconfig` `ns_per_abstract_cycle`). This is the board's **cost model only**. The loader rejects any program whose `abstract_cycles × ns/cycle` exceeds the program's own declared scan period (`LE_ERR_TIMING_BUDGET`), so an unachievable circuit never loads. For **execute-in-place boards this cost must include flash-fetch latency**; for RAM-exec boards it will simply calibrate lower. |
| `LE_PHYSICAL_SLOT_COUNT` | Internal physical storage partitions, always `LE_MAX_CONFIG_SLOTS + 1`. The extra one is a hidden **phantom** slot used to stage uploads so a bad transmission can never overwrite a live config. Board designers must reserve `(LE_MAX_CONFIG_SLOTS + 1) × LE_SLOT_SIZE_BYTES` of flash; circuit designers / compiler / caps / CLI still see only `LE_MAX_CONFIG_SLOTS`. |
| `LE_EXEC_MODE` | `LE_EXEC_IN_PLACE` (default) or `LE_EXEC_IN_RAM`. Every shipped port (STM32, AVR, RP2040/RP2350) executes in place (XIP). `LE_EXEC_IN_RAM` copies the whole image to RAM before binding — an option for large-RAM microprocessors. The compiler output is byte-identical either way; this is purely a board build-time policy. |
